function main(
  ...
  // itemsJson = JSON array from the flow's Select action:
  // [{"id":"1","title":"BSA Monitr Anlyst","jobCode":"1234"}, ...]
  ...
  // ---- Level canonicalization: Roman numerals, ordinals, and words -> digits ----
  ...
  function tokenScore(inputNorm: string, titleNorm: string): number {
    ...
    return Math.round(matched / inputTokens.length);
  }
  ...
  const scored = items.map(item => {
    ...
    const whole = similarity(inputNorm, titleNorm);
    const tokens = tokenScore(inputNorm, titleNorm);
    // Token score weighted higher: users type partial titles far more often
    // than misspelled full ones
    const score = Math.round(whole * 0.35 + tokens * 0.65);
    return { id: item.id, title: item.title, jobCode: item.jobCode, score: score };
  });

  scored.sort((a, b) => b.score - a.score);
  return JSON.stringify(scored.slice(0, 5));
}
